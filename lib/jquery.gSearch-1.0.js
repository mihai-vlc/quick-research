					...
					if (_google_search.results && _google_search.results.length > 0) {
						...
						if(results.length > 0) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								results_html += '<div class="result"><div class="title"><a class="open" href="'+results[i].url+'"><span>'+results[i].title+'</span></a></div><div class="snippet"><span>'+results[i].content+'</span></div><div class="url-cache"><span>'+results[i].url.replace('http://','')+'</span>&nbsp;-&nbsp;<a href="'+results[i].cacheUrl+'" class="open">Cached</a></div></div>';
							...
							if(o.pagination){
								...
								// Set Previous
								if(curPage > 0)
									results_html += ' <li class="prev-next prev"><a class="gpage" href="#" data-page="'+((curPage*1)-1)+'">Previous'+"</a></li> \r";
									
								for (var i = 0; i < cursor.pages.length; i++) {
									
									// Set Page Number Link
									results_html += ' <li class="numbers '+((curPage == i) ? " selected": "")+'"><a class="gpage" href="#" data-page="'+i+'">'+cursor.pages[i].label+"</a></li> \r";									
								}
								
								// Set Next
								if(curPage < (cursor.pages.length-1))
									results_html +=
									' <li class="prev-next next"><a class="gpage" href="#" data-page="'+((curPage*1)+1)+'">Next'+"</a></li> \r";
								results_html += "</ul></div>";
							}
